...
<body>
    ...
    <script>
        ...
        """ % (qr_css_width, qr_css_height, qr_size, qr_css_width, qr_css_height, xurldownload, overlay_url, pdf_url, pdf_url, pdf_url, xurldownload, qr_size, filename, logo_src, layout_json)
    ...
    @http.route([
        '/web/ultimocertificado/<model("res.partner"):partner>/<model("certificado.certificado"):certificado>/<string:strurl>/overlay_js_card35'
    ], type='http', auth="user", website=True)
    def print_certificate_with_qr_overlay_js_card35(self, partner=None, certificado=None, strurl=None, **kwargs):
        """
        Muestra un certificado con QR Card de 3.5cm desde servicio externo.
        Duplica la funcionalidad de overlay_js pero usa https://ctf-qr.onrender.com/card?qr=&cm=3.5
        """
        _logger.info("print_certificate_with_qr_overlay_js_card35 - Iniciando con partner: %s, certificado: %s, strurl: %s" % (partner, certificado, strurl))

        # Buscar tipo de documento dinámicamente
        tipo_documento_info = self._buscar_tipo_documento_dinamico(strurl)
        if not tipo_documento_info:
            return self._return_error_response("Tipo de documento no encontrado")

        strurlruta_final = tipo_documento_info['strurlruta_final']
        xid = tipo_documento_info['xid']
        xuserid = tipo_documento_info['xuserid']
        idcertificado = tipo_documento_info.get('idcertificado')

        # Buscar certificado
        domain = [('cliente_id', '=', xid)]
        if idcertificado:
            domain.append(('id', '=', idcertificado))

        slide_slide_obj = request.env['informes.encuestas.merge'].sudo().search(domain, order='fecha_vigencia desc', limit=1)

        # Fallback con parent_id si no se encuentra
        if not slide_slide_obj:
            try:
                partner = request.env['res.partner'].sudo().browse(cliente_id)
                if partner and partner.parent_id:
                ...
                _logger.error("print_certificate_with_qr_overlay_js_card35 - Error en fallback parent_id: %s" % str(e))
        ...
        # Para JS Card 3.5cm, configuramos el tamaño correspondiente
        qr_size = '3.5cm'
        qr_css_width = 276  # 3.5cm * 78.74 px/cm ≈ 276px
        qr_css_height = '545px'  # Proporcionalmente más alto

        # Nombre del archivo
        if slide_slide_obj.file_name_certificado:
            filename = slide_slide_obj.file_name_certificado.replace('.pdf', '-QR-JS-CARD35.pdf')
        else:
            filename = (slide_slide_obj.codigocliente + '-QR-JS-CARD35.pdf') if slide_slide_obj.codigocliente else 'CERTIFICADO_QR_JS_CARD35.pdf'

        # Construir URLs auxiliares
        import urllib.parse
        # URL del servicio externo para generar la imagen QR con parámetro cm=3.5
        external_qr_url = 'https://ctf-qr.onrender.com/card?qr=' + urllib.parse.quote(xurldownload) + '&cm=3.5'
        overlay_url = base_url + '/web/ultimocertificado/' + str(strurlruta_final) + '/' + str(xid) + '/' + str(xuserid) + '/' + str(strurl) + '/overlay'

        # HTML con diseño de dos columnas
        html_content = """
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <title>Certificado con QR Card 3.5cm - %s</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: #f5f7fa; }
        .container { max-width: 1400px; margin: 0 auto; padding: 20px; display: grid; grid-template-columns: %dpx 1fr; gap: 20px; min-height: 100vh; }
        .card { background: white; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); padding: 20px; }
        .title { color: #2c3e50; margin-bottom: 15px; font-size: 1.2em; font-weight: 600; }
        .qr-container { text-align: center; }
        .qr-image { width: %dpx; height: %s; border: 2px solid #e74c3c; border-radius: 8px; object-fit: contain; }
        .qr-info { margin-top: 15px; font-size: 0.9em; color: #7f8c8d; }
        .qr-size { font-weight: bold; color: #e74c3c; font-size: 1.1em; }
        .pdf-viewer { width: 100%%; height: 80vh; border: none; border-radius: 8px; }
        .actions { margin-top: 15px; }
        .btn { display: inline-block; padding: 10px 20px; margin: 5px; text-decoration: none; border-radius: 6px; font-weight: 500; transition: all 0.3s; }
...
<body>
    <div class="container">
        <!-- Columna izquierda: QR Card externo -->
        <div class="card">
            <h3 class="title">QR Card Externo</h3>
            <div class="qr-container">
                <img src="%s" alt="QR Code Card" class="qr-image" />
                <div class="qr-info">
                    <div class="qr-size">Tamaño: %s</div>
                    <p><strong>Servicio:</strong> Externo</p>
                    <p><strong>URL:</strong> https://ctf-qr.onrender.com/card</p>
                    <p><strong>Verificación:</strong> <a href="%s" target="_blank" style="color: #3498db;">%s</a></p>
                </div>
        ...
        """ % (filename, qr_css_width, qr_css_width, qr_css_height, external_qr_url, qr_size, xurldownload, xurldownload, overlay_url, external_qr_url, pdf_url, pdf_url)
