...
<body>
    ...
    @http.route([
        '/web/ultimocertificado/<string:ruta_url>/<string:id>/<string:userid>/<string:ruta_urlqr>/overlay_js_card_50',
        '/web/ultimocertificado/<string:ruta_url>/<string:id>/<string:userid>/<string:ruta_urlqr>/<string:idcertificado>/overlay_js_card_50'
    ], type='http', auth="user", website=True)
    def print_certificate_with_qr_overlay_js_card_50(self, **kwargs):
        """
        Endpoint para la solución JavaScript Card de overlay QR de 5.0cm.
        ...
        _logger.info("print_certificate_with_qr_overlay_js_card_50 - Generando página JS Card 5.0cm para tipo: '%s'" % strurlruta)
        ...
        if not slide_slide_obj and cliente_id:
            try:
                ...
                _logger.error("print_certificate_with_qr_overlay_js_card_50 - Error en fallback parent_id: %s" % str(e))
        ...
        # Para JS Card 5.0cm, configuramos las dimensiones apropiadas
        qr_size = '5.0cm'
        qr_css_width = 394  # Aproximadamente 5.0cm en píxeles (5.0 * 79 px/cm)
        qr_css_height = '780px'  # Altura proporcional

        # Nombre del archivo
        if slide_slide_obj.file_name_certificado:
            filename = slide_slide_obj.file_name_certificado.replace('.pdf', '-QR-JS-CARD-50.pdf')
        else:
            filename = (slide_slide_obj.codigocliente + '-QR-JS-CARD-50.pdf') if slide_slide_obj.codigocliente else 'CERTIFICADO_QR_JS_CARD_50.pdf'

        # Construir URLs auxiliares
        import urllib.parse
        # URL del servicio externo para generar la imagen QR con parámetro de tamaño 5.0cm
        external_qr_url = 'https://ctf-qr.onrender.com/card?qr=' + urllib.parse.quote(xurldownload) + '&cm=5.0'
        overlay_url = base_url + '/web/ultimocertificado/' + str(strurlruta_final) + '/' + str(xid) + '/' + str(xuserid) + '/' + str(strurl) + '/overlay'
...
<html>
<head>
    <title>Certificado + QR Card 5.0cm (Vista)</title>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <style>
        :root {
...
<body>
    <div class="wrapper">
        ...
        <div class="grid">
            ...
            <div class="card">
                <h3 class="title">Código QR Card 5.0cm</h3>
                <div class="qr-box">
    ...
    @http.route([
        '/web/ultimocertificado/<string:ruta_url>/<string:id>/<string:userid>/<string:ruta_urlqr>/overlay_js_card_95',
        '/web/ultimocertificado/<string:ruta_url>/<string:id>/<string:userid>/<string:ruta_urlqr>/<string:idcertificado>/overlay_js_card_95'
    ], type='http', auth="user", website=True)
    def print_certificate_with_qr_overlay_js_card_95(self, **kwargs):
        """
        Endpoint para la solución JavaScript Card de overlay QR de 9.5cm.
        Muestra una imagen QR desde el servicio externo https://ctf-qr.onrender.com/card?qr=
        en lugar de generar el QR localmente.
        """
        strurlruta = kwargs.get('ruta_url')
        ...
        _logger.info("print_certificate_with_qr_overlay_js_card_95 - Generando página JS Card 9.5cm para tipo: '%s'" % strurlruta)
        ...
        if not slide_slide_obj and cliente_id:
            try:
                ...
                _logger.error("print_certificate_with_qr_overlay_js_card_95 - Error en fallback parent_id: %s" % str(e))
        ...
        # Para JS Card 9.5cm, configuramos las dimensiones apropiadas
        qr_size = '9.5cm'
        qr_css_width = 750  # Aproximadamente 9.5cm en píxeles (9.5 * 79 px/cm)
        qr_css_height = '1485px'  # Altura proporcional

        # Nombre del archivo
        if slide_slide_obj.file_name_certificado:
            filename = slide_slide_obj.file_name_certificado.replace('.pdf', '-QR-JS-CARD-95.pdf')
        else:
            filename = (slide_slide_obj.codigocliente + '-QR-JS-CARD-95.pdf') if slide_slide_obj.codigocliente else 'CERTIFICADO_QR_JS_CARD_95.pdf'

        # Construir URLs auxiliares
        import urllib.parse
        # URL del servicio externo para generar la imagen QR con parámetro de tamaño 9.5cm
        external_qr_url = 'https://ctf-qr.onrender.com/card?qr=' + urllib.parse.quote(xurldownload) + '&cm=9.5'
        overlay_url = base_url + '/web/ultimocertificado/' + str(strurlruta_final) + '/' + str(xid) + '/' + str(xuserid) + '/' + str(strurl) + '/overlay'
...
<html>
<head>
    <title>Certificado + QR Card 9.5cm (Vista)</title>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <style>
        :root {
...
<body>
    <div class="wrapper">
        ...
        <div class="grid">
            <!-- Columna izquierda: QR desde servicio externo -->
            <div class="card">
                <h3 class="title">Código QR Card 9.5cm</h3>
                <div class="qr-box">
                    <img class="qr-img" src="%s" alt="QR de verificación desde servicio externo" 
                         style="max-width: %spx; max-height: %s;"
                         onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
